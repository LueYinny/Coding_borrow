...
FR::FR()
{
	...
	fd.LoadModel("models/face_seg", "0548", "segnet0_relu8_fwd_output", 128);
	fe.LoadModel("models/mobileface", "0000", "conv_6dw7_7_conv2d_output", 112);
	//fe.LoadModel("models/mod", "0000", "s5_global_conv_output", 128);
...
std::string FR::recProbe(cv::Mat image, std::string task) {
    ...
    //std::cout<<"similarity = "<<result.second<<std::endl;
	if (result.first >= 0 && result.second > REC_THRESHOLD) {
		return this->gallery_names[result.first];
	}
	else {
		return "unknown";
	}
